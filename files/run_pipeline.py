...
from pipeline.genome_fetcher   import search_taxon_accessions, fetch_sequences, get_close_relatives
...
def _parse_args():
    ...
    p.add_argument("--max-relative-seqs", type=int, default=3,
                   help="Max close-relative sequences to fetch (default: 3)")
...
def stage_roi(args, target_recs, excl_recs) -> List[ROICandidate]:
    ...
    log.info("Scanning for unique ROIs (window=%d step=%d min_uniqueness=%.2f)",
             args.roi_window, args.roi_step, args.min_uniqueness)
...
def stage_design(args, rois) -> List[PrimerSet]:
    ...
    for roi in rois:
        ...
        log.info("ROI %s:%d-%d → %d primer sets designed",
                 roi.record_id, roi.start, roi.end, len(sets))
...
def stage_blast(args, primer_sets) -> List[PrimerSet]:
    """Stage 4: BLAST validation."""
    log = logging.getLogger("stage.blast")

    config.BLAST_WORKERS = args.blast_workers
